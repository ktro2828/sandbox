...
inline std::vector<std::string> split(const std::string & s, char delim)
{
  ...
  for (char ch : s) {
    if (ch == delim) {
      if (!item.empty()) elems.emplace_back(item);
      item.clear();
    } else {
      item += ch;
    }
...
public:
  Arguments(
    const std::unordered_map<std::string, std::vector<std::string>> & arguments,
...
public:
  Arguments parseArgs(
    int argc, char const * const * const argv, bool allowEmptyPositionalArguments = false) const
  {
    ...
    if (!allowEmptyPositionalArguments) {
      if (_positional.size() != passed_positional_count) {
        std::cerr << "required positional arguments is " << _positional.size() << ". but "
                  << passed_positional_count << " was passed." << std::endl;
        std::exit(EXIT_FAILURE);
      }
    }
    for (const auto & i : _positional) {
      if (itr == std::end(args)) break;
...
public:
  void addArgument(
    const std::vector<std::string> & name, const std::string & help,
