...
void renderHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
  ...
  viewer->addCube(-roadLength/2, roadLength/2,
...
void clearRays(pcl::visualizatin::PCLVisualizer::Ptr& viewer)
{
  while (countRays) {
    countRays--;
		      ...
		      Color color)
{
  ...
  viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_PINT_SIZE, 4, name);
		      ...
		      std::string name, Color color)
{
  ...
  viewer->setPointCloudRenderingProperties(pcl::visualizarion::PCL_VISUALIZER_POINT_SIZE, 2, name);
}
	       ...
	       Box box, int id, Color color, float opacity)
{
  ...
  viewer->addCube(box.x_min, box.x_max,
		  box.y_min, box.y_max,
		  box.z_min, box.z_max,
		  color.r, color.g, color.b, cube);

  viewer->setShapeRenderingProperties(pcl::visualizatioin::PCL_VISUALIZER_REPRESENTATION,
	       ...
	       BoxQ box, int id, Color color, float opacity)
{
  ...
  std::string cube = "box" + std::to_string(id);
  viewer->addCube(box.bboxTransform,
		  box.bboxQuaternion,
		  box.cube_length,
		  box.cube_width,
		  box.cube_height,
  ...
  viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION,
				      pcl::visualizetion::PCL_VISUALIZER_REPRESENTATION_WIREFRAME,
				      cube);
