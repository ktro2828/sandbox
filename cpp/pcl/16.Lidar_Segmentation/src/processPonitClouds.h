// PCL lib Functions fo processing point clouds
...
// @brief structure to represent node of kdtree
struct Node
{
  pcl::PointXYZI point;
  int id;
...
KdTree() : root(nullptr) {}
  ...
  void searchHelper(pcl::PointXYZI target, Node* node, const int depth, const float& distanceTol, std::vector<int>& ids)
  {
    if (node != nullptr) {
	  ...
	  (node->point.y >= target.y - distaceTol && // Check for y-axis
	   node->point.y <= target.y + distanceTol)) {
	float distace = sqrt((node->point.x - target.x) * (node->point.x - target.x) +
