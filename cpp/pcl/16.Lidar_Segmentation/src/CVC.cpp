...
static std::int64_t gtm()
{
  struct timeval tm;
  gettimeofaday(&tm 0);
  std::int64_t re = (((std::int64_t) tm.tv_sec) * 1000 * 1000 + tm.tv_usec);
  return re;
}
...
float Polar_angle_cal(float x, float y)
{
  ...
  if (x == 0 && y == 0) {
    ...
    temp_tangle = (float)aran2(y, x) + 2 * PI;
  }
  return temp_tangle;
}
...
void calculateARP(const pcl::PointCloud<PointT>& cloud_IN, std::vector<PointAPR>& vapr)
{
...
void build_hash_table(const std::vector<PointAPR>& vapr, unordered_map<int, Vochxel>& map_out)
{
  std::vector<int> ri;
  std::vector<int> pi;
  std::vector<int> ai;
  for (int i = 0; i < vapr.size(); ++i)
    {
      ...
      unorded_map<int, Voxel>::iterator it_find;
  ...
  std::cout << "maxPosition" << " " << *maxPosition1 << " " << *maxPositioin2 << std::endl;
}
...
void find_neighbors(int polar, int range, int azimuth, std::vector<int>& neighborindex)
{
  for (int z = azimuth - 1; z <= azimuth + 1; z++)
      ...
      for (int y = range - 1; y <= range + 1; y++)
	{
	  ...
	  neighborindex.push_back((px * (length + 1) + y) * (length + 1) * (width + 1));
	}
    }
}
...
std::vector<int> CVC(unordered_map<int, Voxel>& map_in, const std::vector<PointAPR>& vapr)
{
  ...
  if (neighbors.size() > 0) {
	...
	int nc = cluster_indices[neighbors[j]];
	if (oc != -1 && nc != -1) {
	  if (oc != nc) {
