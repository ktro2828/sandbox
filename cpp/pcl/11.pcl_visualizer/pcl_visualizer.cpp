...
boost::shared_ptr<pcl::visualization::PCLVisualizer> customColourVis(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
{
    ...
    viewer->addPointCloud<pcl::PointXYZ>(cloud, single_color, "sample cloud"); //点云自定义颜色
    viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "sample cloud");
    viewer->addCoordinateSystem(1.0);
    viewer->initCameraParameters();
    return (viewer);
...
boost::shared_ptr<pcl::visualization::PCLVisualizer> normalsVis(pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloud, pcl::PointCloud<pcl::Normal>::ConstPtr normals)
{
    ...
    viewer->addPointCloudNormals<pcl::PointXYZRGB, pcl::Normal>(cloud, normals, 10, 0.05, "normals"); //每10个点显示一个法线，长度0.02
    viewer->addCoordinateSystem(1.0);
    viewer->initCameraParameters();
    return (viewer);
}
...
int main(int argc, char **argv)
{
    ...
    if (pcl::console::find_argument(argc, argv, "-s") >= 0) //return index of found argument or -1 if arguments do not appear in list
    {
        simple = true;
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr point_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGB>); //带有颜色
