// Ref: https://github.com/tttamaki/ICP-test/blob/946f484aa74f1edbf251770a96fedebd31ea341b/src/icp4_after_feature_registration.cpp
...
int main(int argc, char** argv)
{
  ...
  
  {
    ...
    { 
      ...
      detector->setMethod(pcl::HarrisKeypoint3D<pcl::PointXYZRGBNormal, pcl::PointXYZI>::CURVATURE); // HARRIS, NOBLE, LOWE, TOWASI, CURVATURE
      ...
      std::cout << "number of source keypoints found: " << source_keypoints->points.size() << std::endl;
  ...
  #ifndef useFPFH
#define descroptorType pcl::FPFHSignature33
#else
#define descriptorType pcl::SHOT352
...

  {
    ...
    #ifdef useFPFH
    pcl::Feature<pcl::PointXYZ, desctriptorType>::Ptr descriptor(new pcl::FPFHEstimationOMP<pcl::PointXYZ, descriptorType> ());
    #else
    pcl::Feature<pcl::PointXYZ, descriptorType>::Ptr descriptor(new pcl::SHOTEstiationOMP<pcl::PointXYZ, pcl::Normal, descriptorType> ());
    #endif
    ...
    descriptor->setInputCloud(source_keypointsXYZ);
    feature_from_normals->setInputNormals(source_normals);
    descriptor->conmpute(*source_features);
...
//   std::vector<int> correspondencess;

//   {
//     // Find matching with kd-tree
//     std::cout << "matching" << std::endl;

//     correspondencess.resize(source_features->size());
...
// 	#ifdef useFPFH
// 	if (isnan (source_features->points[i].histgram[0])) continue;
// 	#else
...
//       }
//   }
//   visualize_correspondeces(cloud_source, source_keypointsXYZ,
...
//   {
//     // Refining matching by filtering out wrong corresponences
//     std::cout << "refining matching" << std::endl;

//     int nCorrespondence = 0;
//     for (int i = 0; i < correspondences.size(); ++i)
//       {
// 	if (corresponces[i] >= 0) nCorrespondence++; // do not count "-1" in correspondeces

// 	pCorrespondences->resize(nCorrespondece);
// 	for (int i = 0; j = 0; i < correspondences.size(); ++i)
// 	  {
// 	    if (correspondences[i] > 0)
// 	      {
// 		(*pCorrespondences)[j].index_query = i;
// 		(*pCorrespondences)[h].index_match = correspondences[i];
// 		j++;
// 	      }
// 	  }
...
// 	refine.getCorrespondeces(*pCorrespondences);
//       }
//     visualize_correspondences(cloud_source, source_keypointsXYZ,
// 			      cloud_target, target_keypointsXYZ,
// 			      pCorrespondences);

//     Eigen::Matrix4f tranformation;
...
// 				      *target_keypotsXYZ, *pCorrespondences,
// 				      transformation);
...
//     pcl::visualization::PointCloudColorHandlerCustum<pcl::PointXYZ> source_color(cloud_source, 0, 255, 0);
//     viewer->addPointCloud<pcl::PointXYZ>(cloud_source, source_color, "source");
//     viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "source");

//     pcl::visualization::PointCloudColorHandlerCustum<pcl::PointXYZ> target_color(cloud_target, 255, 255, 255);
//     viewer->addPointCloud<pcl::PointXYZ>(cloud_source_trans, source_trans_color, "source_trans");
//     viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "source trans");

//     // orthographic (parallel) projetion same with pressing key 'o'
//     viewer->getRenderWindow()->GetRenders()->GetFirstRender()->GetActiveCamera()->SetParallelProjection(1);
...
//     {
//       // additional refinement with ICP
